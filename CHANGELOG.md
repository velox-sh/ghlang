...
- `render::template` substitution engine: `{{ key }}` markers, whitespace tolerant, XML-escaped values by default, `_raw` suffix bypass for pre-rendered SVG fragments, `TemplateError::MissingKey` for unresolved markers. 8 unit tests.
- `render::theme` TOML loader with builtin `default`, `dark`, `monokai` themes (embedded via `include_str!`), plus `Theme::load(name_or_path)` for user-provided themes from disk. `Theme::language_colour()` resolves theme override -> linguist colour map (40+ common langs in `src/static/languages.toml`) -> `colors.fallback`.
